...
describe("HypNative", function () {
  ...
  describe("deployment & upgradeability", function () {
    ...
    it("Should deploy an upgradeable contract correctly", async function () {
      ...
      const hypNativeImpl = await (hre.viem.deployContract as any)(
        "HypNativeV1",
        [mockMailbox.address],
        { walletClient: deployer },
      );
      ...
      const hypNativeProxy = await (hre.viem.deployContract as any)(
        "TransparentUpgradeableProxy",
        [hypNativeImpl.address, proxyAdmin.address, ""],
        { walletClient: deployer },
      );
      ...
      await hypNative.write.setMaxKadenaChainId([MAX_KADENA_CHAIN_ID]);

      // Verify version
      expect(await hypNative.read.version()).to.equal(1);
    });
    ...
    it("Should correctly upgrade to V2", async function () {
      ...
      const hypNativeV1 = await hre.viem.getContractAt(
        "HypNativeV1",
        hypNativeProxy.address,
      );

      // Initialize V1
      await hypNativeV1.write.initialize([
        mockIGP.address,
        mockISM.address,
        deployer.account!.address,
      ]);
      await hypNativeV1.write.setMaxKadenaChainId([MAX_KADENA_CHAIN_ID]);

      // Verify initial balance
      const initialBalance = await hypNativeV1.read.balanceOf([
        alice.account!.address,
      ]);

      // Set max Kadena chain ID to a differet value
      const newMaxChainId = 30;
      await hypNativeV1.write.setMaxKadenaChainId([newMaxChainId], {
        account: deployer.account,
      });

      expect(await hypNativeV1.read.getMaxKadenaChainId()).to.equal(
        newMaxChainId,
      );

      // Deploy V2 implementation
      const hypNativeImplV2 = await (hre.viem.deployContract as any)(
        "HypNativeV2",
        [mockMailbox.address],
        { walletClient: deployer },
      );
      ...
      await proxyAdmin.write.upgrade([
        hypNativeProxy.address,
        hypNativeImplV2.address,
      ]);

      // Get V2 contract instance
      const hypNativeV2 = await hre.viem.getContractAt(
        "HypNativeV2",
        hypNativeProxy.address,
      );

      // Verify upgraded version
      expect(await hypNativeV2.read.version()).to.equal(2);

      // Verify balance is preserved after upgrade
      const upgradedBalance = await hypNativeV2.read.balanceOf([
        alice.account!.address,
      ]);
      expect(upgradedBalance).to.equal(initialBalance);

      // Verify balance after transfer
      const newBalance = await hypNativeV2.read.balanceOf([
        alice.account!.address,
      ]);
      expect(newBalance).to.equal(initialBalance);

      // Verify max Kadena chain ID is preserved after upgrade
      expect(await hypNativeV2.read.getMaxKadenaChainId()).to.equal(
        newMaxChainId,
      );
    });
  ...
  describe("token minting", function () {
    beforeEach(async function () {
      ...
      hypNative = await hre.viem.getContractAt(
        "HypNative",
        hypNativeProxy.address,
      );
    ...
    it("does not have external minting function", async function () {
      await expect(hypNative.write.mint()).to.be.rejectedWith(
  ...
  describe("transferRemote", function () {
    beforeEach(async function () {
      ...
      await hypNative.write.enrollRemoteRouter([
        DESTINATION_DOMAIN,
    ...
    context("Success Test Cases", function () {
      it("Should update state and emit correct events without custom gas config", async function () {
        ...
        const balanceBefore = await hypNative.read.balanceOf([
          alice.account!.address,
        ]);
        const balanceBeforeContract = await hypNative.read.balanceOf([
          hypNative.address,
        ]);

        // Get required amount from the contract
        requiredAmount = await hypNative.read.quoteGasPayment([
          DESTINATION_DOMAIN,
        ]);
        ...
        const tx = await hypNative.write.transferRemote(
        ...
        const balanceAfter = await hypNative.read.balanceOf([
          alice.account!.address,
        ]);
        const balanceAfterContract = await hypNative.read.balanceOf([
          hypNative.address,
        ]);

        // Check ETH spent (gas + required amount)
        expect(balanceBeforeEth - balanceAfterEth).to.equal(
          gasCost + amount + requiredAmount,
        );
        ...
        const sentEvents = await hypNative.getEvents.SentTransferRemote({
      ...
      it("Should update state and emit correct events with custom gas config", async function () {
        ...
        await hypNative.write.setDestinationGas([
          [
            {
      ...
      it("Should handle different Kadena address formats correctly", async function () {
        ...
        const tx = await hypNative.write.transferRemote(
          ...
          { value: amount + requiredAmount, account: alice.account },
        );
      ...
      it("Should handle transfers with different token amounts correctly", async function () {
        ...
        const smallAmount = parseEther("0.000001"); // Tiny amount
        const largeAmount = parseEther("1000"); // Large amount

        // Test small amount transfer
        const txSmall = await hypNative.write.transferRemote(
          [
            DESTINATION_DOMAIN,
            ENCODED_KADENA_RECIPIENT,
            smallAmount,
          ...
          { value: smallAmount + requiredAmount, account: alice.account },
        );
        ...
        const sentEventsSmall = await hypNative.getEvents.SentTransferRemote({
        ...
        const txLarge = await hypNative.write.transferRemote(
          [
            DESTINATION_DOMAIN,
            ENCODED_KADENA_RECIPIENT,
            largeAmount,
          ...
          { value: largeAmount + requiredAmount, account: alice.account },
        );
        ...
        const sentEventsLarge = await hypNative.getEvents.SentTransferRemote({
    ...
    context("Error Test Cases", function () {
      it("Should revert when attempting to transfer more tokens than balance", async function () {
        ...
        const aliceBalance = await hypNative.read.balanceOf([
          alice.account!.address,
        ]);

        // Attempt to transfer more than the balance
        const excessiveAmount = aliceBalance + parseEther("1");

        // Expect the transaction to revert
        await expect(
          hypNative.write.transferRemote(
            [
              DESTINATION_DOMAIN,
              ENCODED_KADENA_RECIPIENT,
              excessiveAmount,
            ...
            { value: excessiveAmount + requiredAmount, account: alice.account },
          ),
      ...
      it("Should revert when insufficient gas payment is provided", async function () {
        ...
        await expect(
          hypNative.write.transferRemote(
            ...
            { value: amount + insufficientGas, account: alice.account },
          ),
      ...
      it("Should revert when transfer amount is zero", async function () {
        ...
        ).to.be.rejectedWith("HypNative: amount must be greater than 0");
      });
  ...
  describe("handle", function () {
    ...
    beforeEach(async function () {
      ...
      balanceBefore = await hypNative.read.balanceOf([bob.account!.address]);
      ...
      // Transfer tokens to hypNative
      const transferAmount = parseEther("1000");

      const [fromWallet] = await hre.viem.getWalletClients();
      await fromWallet.sendTransaction({
        account: deployer.account!.address,
        to: hypNative.address,
    ...
    context("Success Test Cases", function () {
      it("Should mint tokens to recipient and emit ReceivedTransferRemote event", async function () {
        ...
        const tx = await hypNative.write.handle(
        ...
        const events = await hypNative.getEvents.ReceivedTransferRemote({
      ...
      it("Should handle messages with small token amounts efficiently", async function () {
        const smallAmount = parseEther("0.000001"); // Very small amount
        ...
        const balance = await hypNative.read.balanceOf([bob.account!.address]);
      ...
      it("Should handle messages with large token amounts efficiently", async function () {
        const largeAmount = parseEther("1000"); // Very large amount
      ...
      it("Should handle messages with different recipients efficiently", async function () {
        const transferAmount1 = parseEther("50");
        ...
        const transferAmount2 = parseEther("100");
        ...
        const balanceBeforeAlice = await hypNative.read.balanceOf([
          alice.account!.address,
        ]);

        // Process first message
        const tx1 = await hypNative.write.handle(
        ...
        // Verify event was emitted
        const events1 = await hypNative.getEvents.ReceivedTransferRemote({
        ...
        // Process second message and track gas
        const tx2 = await hypNative.write.handle(
        ...
        const balanceAfterBob = await hypNative.read.balanceOf([
          bob.account!.address,
        ]);
        expect(balanceAfterBob).to.equal(balanceBefore + transferAmount1);
        const balanceAfterAlice = await hypNative.read.balanceOf([
          alice.account!.address,
        ]);
        ...
        // Verify event was emitted
        const events2 = await hypNative.getEvents.ReceivedTransferRemote({
  ...
  describe("setMaxKadenaChainId", function () {
    ...
    context("Success Test Cases", function () {
      it("Should update maxKadenaChainId when called by owner", async function () {
        // Check initial value
        const initialMaxChainId = await hypNative.read.getMaxKadenaChainId();
        expect(initialMaxChainId).to.equal(MAX_KADENA_CHAIN_ID);

        // Update the max chain ID
        const newMaxChainId = 10;
        const tx = await hypNative.write.setMaxKadenaChainId([newMaxChainId], {
          account: deployer.account,
        });
        ...
        const updatedMaxChainId = await hypNative.read.getMaxKadenaChainId();
        expect(updatedMaxChainId).to.equal(newMaxChainId);

        // Check the event
        const events = await hypNative.getEvents.MaxKadenaChainIdSet({
          fromBlock: txReceipt.blockNumber,
          toBlock: txReceipt.blockNumber,
        });
        expect(events).to.have.lengthOf(1);
      ...
      it("Should allow increasing the maxKadenaChainId", async function () {
        ...
        const tx = await hypNative.write.setMaxKadenaChainId([smallerValue], {
          account: deployer.account,
        });
        ...
        // Verify the update
        expect(await hypNative.read.getMaxKadenaChainId()).to.equal(
          smallerValue,
        );

        // Now increase to a higher value
        const higherValue = 25;
        await hypNative.write.setMaxKadenaChainId([higherValue], {
          account: deployer.account,
        });
      ...
      it("Should allow setting maxKadenaChainId to zero", async function () {
        // Set to zero
        const tx = await hypNative.write.setMaxKadenaChainId([0], {
          account: deployer.account,
        });
        ...
        // Verify the update
        expect(await hypNative.read.getMaxKadenaChainId()).to.equal(0);
      ...
      it("Should allow updating maxKadenaChainId multiple times", async function () {
        ...
        for (const value of values) {
          await hypNative.write.setMaxKadenaChainId([value], {
            account: deployer.account,
          });

          // Verify each update
          expect(await hypNative.read.getMaxKadenaChainId()).to.equal(value);
        }
      });
      ...
      it("Should handle uint16 max value correctly", async function () {
        ...
        await hypNative.write.setMaxKadenaChainId([maxUint16], {
          account: deployer.account,
        });

        // Verify it was set correctly
        expect(await hypNative.read.getMaxKadenaChainId()).to.equal(maxUint16);
      });
    }); // End of Success Test Cases
    ...
    context("Error Test Cases", function () {
      it("Should revert when called by non-owner account", async function () {
        ...
        await expect(
          hypNative.write.setMaxKadenaChainId([10], {
            account: alice.account,
          }),
      ...
      it("Should revert if the new value is the same as the current alue", async function () {
        // Attempt to update from a non-owner account
        await expect(
          hypNative.write.setMaxKadenaChainId([MAX_KADENA_CHAIN_ID], {
            account: deployer.account,
          }),
        ).to.be.rejectedWith(
  ...
  describe("getMaxKadenaChainId", function () {
    ...
    context("Success Test Cases", function () {
      it("Should return the initialized value", async function () {
        ...
        const maxChainId = await hypNative.read.getMaxKadenaChainId();
      ...
      it("Should return updated value after setting", async function () {
        // Update the value
        const newValue = 10;
        await hypNative.write.setMaxKadenaChainId([newValue], {
          account: deployer.account,
        });
      ...
      it("Should return value directly from storage slot", async function () {
        ...
        const getterValue = await hypNative.read.getMaxKadenaChainId();
      ...
      it("Should maintain values after contract interactions", async function () {
        // Set a known value
        const testValue = 12;
        await hypNative.write.setMaxKadenaChainId([testValue], {
          account: deployer.account,
        });
